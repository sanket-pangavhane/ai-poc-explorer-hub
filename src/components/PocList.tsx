...
const PocList = () => <div className="w-full max-w-2xl mx-auto bg-white p-6 shadow-lg mt-8 border-0 rounded-xl">
    <h2 className="text-xl md:text-2xl font-semibold mb-6 text-[#1A1F2C]">Proofs of Concept</h2>
    <ul className="divide-y divide-gray-100">
      {mockPocs.map((poc, idx) => <li key={poc.name} className="flex items-center justify-between py-4 px-2 rounded-lg transition-colors duration-200 group hover:bg-[#f1f0fb]">
          <span className="text-lg text-gray-900 font-medium">{poc.name}</span>
          <button className="relative flex items-center gap-2 px-5 py-2 rounded-lg font-semibold bg-[#9b87f5] text-white overflow-hidden transition-colors duration-500 focus:outline-none
            ...
            <span className="relative z-10 flex items-center justify-center">
              <span className="flex items-center justify-center rounded-full bg-[#1e2040] p-1" style={{
            boxShadow: "0 1px 4px 0 rgba(30,32,64,0.11)",
            backgroundColor: "#1e2040"
          }}>
                <ArrowRight size={16} className="rounded-full" color="#fde047" strokeWidth={2.5} style={{
              background: "#fde047",
              borderRadius: "9999px",
              padding: 2,
            ...
            <span className="absolute left-0 top-0 w-full h-full rounded-lg z-0 pointer-events-none" style={{
